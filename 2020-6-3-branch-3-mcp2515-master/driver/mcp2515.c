...
static void mcp2515_write_can_bandrate(struct spi_device *spi)
{
	...
	CNF1_VALUE = mcp2515_read_reg(spi, CNF1);
	printk("CNF1 = %x\n",CNF1_VALUE);
	CNF2_VALUE = mcp2515_read_reg(spi, CNF2);
	printk("CNF2 = %x\n",CNF2_VALUE);
	CNF3_VALUE = mcp2515_read_reg(spi, CNF3);
	printk("CNF3 = %x\n",CNF3_VALUE);
...
{		
	...
	if (chip->txbout != chip->txbin) {
		...
		mcp2515_write_can_id(spi, (uint32_t)tx_buf+1, frame->can_id, 0, 1);
	...
	/*
	TXBnCTRL = mcp2515_read_reg(spi, TXBCTRL(pos));
	printk("TXBnCTRL = %x\n",TXBnCTRL);	
...
static int mcp2515_hw_reset(struct spi_device *spi)
{
	...
	//st1 = mcp2515_read_reg(spi, CANSTAT);
	//st2 = mcp2515_read_reg(spi, CANCTRL);
...
static int mcp2515_open (struct inode *inode, struct file *filp)
{
	...
	for(i = 0; i < 2; i++) {
		mcp2515_write_reg(spi, RXMSIDH(i), 0x00);
		mdelay(1);
		mcp2515_write_reg(spi, RXMSIDL(i), 0x00);
		mdelay(1);
		mcp2515_write_reg(spi, RXMEID8(i), 0x00);
		mdelay(1);
		mcp2515_write_reg(spi, RXMEID0(i), 0x00);
		mdelay(1);
	}
...
static ssize_t mcp2515_read(struct file *filp, char __user *buf, size_t count, loff_t * lof)
{
	...
	while (chip->rxbin == chip->rxbout) {
		//printk("mcp2515_read-2\n");
		if (filp->f_flags & O_NONBLOCK)
		{
			//printk("mcp2515_read-2.1\n");
			return -EAGAIN;
		}
		//printk("mcp2515_read-3\n");

		//canintf = mcp2515_read_reg(spi, 0x60);
		//printk("RXB0CTRL = %x\n",canintf);

		if (wait_event_interruptible(chip->rwq, (chip->rxbin != chip->rxbout)))
		{
			//printk("mcp2515_read-2.2\n");
			return -ERESTARTSYS;
		}
		//printk("mcp2515_read-4\n");
		//canintf = mcp2515_read_reg(spi, CANINTF);
		//printk("CANINTE = %x\n",canintf);
	}
...
static int __devinit mcp2515_probe(struct spi_device *spi)
{
    ...
    ret = request_irq(IRQ_EINT(12), mcp2515_irq, IRQF_DISABLED | IRQF_TRIGGER_FALLING, DEVICE_NAME, spi);
	if (ret < 0) {
