...
static uint8_t mcp2515_read_reg(struct spi_device *spi, uint8_t reg)
{
	...
	ret = spi_write_then_read(spi, tx_buf, 2, rx_buf, 1);
		printk ("777777777777777777777777777777777777777777\n");
	if (ret < 0) {
		val = 0;
		printk("MCP2515: Read register error!\n");
	} else{
...
static int mcp2515_hw_reset(struct spi_device *spi)
{
	...
	if (ret < 0)
		...
		printk ("666666666666666666666666666666666666\n");
	mdelay(10);
...
static int __devinit mcp2515_probe(struct spi_device *spi)
{
	...
	printk ("11111111111111111111111111111111111111111111111111111111111111111111111\n");
	...
	if (mcp2515_major) {
		...
		printk ("22222222222222222222222222222222222222222222222\n");	
	if (ret < 0) {
		printk("MCP2515: Register char device region (%d: %d) failed! (ret = %d)\n", MAJOR(devno), MINOR(devno), ret);
		goto error_register;
	}
		printk ("33333333333333333333333333333333333333333333333333\n");
	...
	if (ret < 0) {
		...
		printk ("444444444444444444444444444444444444444444444\n");
	chip->class = class_create(THIS_MODULE, "mcp2515_class");
		printk ("41414141414141414141414141414141\n");
	...
	device_create(chip->class, NULL, devno, NULL, DEVICE_NAME);
		printk ("42424242424242424242424242424242422442424242\n");
	ret = mcp2515_hw_reset(spi);
		printk ("5555555555555555555555555555555555555555\n");
	if (!ret)
		goto error_reset;
